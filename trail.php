...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $role = "pharmacist"; // Set role as 'pharmacist'
    $errors = [];
    ...
    if (empty($errors)) {
        ...
        $stmt = $conn->prepare("INSERT INTO users (fullname, email, password, address, place, qualification, role) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("sssssss", $name, $email, $hashed_password, $address, $place, $qualification, $role);

        if ($stmt->execute()) {
            $_SESSION['email'] = $email;
            $_SESSION['role'] = $role;
            header('Location: otp_email.php'); // Redirect to OTP verification
            exit;
        } else {
