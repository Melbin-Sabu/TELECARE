...
$errors = [];
    ...
    if (empty($errors)) {
        ...
        // Database connection
        $connection = new mysqli('localhost', 'root', '', 'telecare+');

        if ($connection->connect_error) {
            die("Connection failed: " . $connection->connect_error);
        }

        // SQL Insert query (make sure column names match the table schema)
        $sql = "INSERT INTO signup (full_name, email, password, permanent_address, place, role) 
                VALUES (?, ?, ?, ?, ?, ?)";

        $stmt = $connection->prepare($sql);
        $stmt->bind_param("ssssss", $name, $email, $hashed_password, $address, $place, $role);
